<form method="POST" id="setting-form">
	...
	<div class="grid grid-cols-5 -mx-2">
		...
		<aside class="sticky top-0 my-2 px-2 w-full overflow-hidden invisible lg:visible">
			...
			<div class="w-full bg-white shadow-sm rounded-lg overflow-hidden">
				...
				<ul class="flex flex-col w-full p-4">
					...
					<?php foreach ( $features as $key => $feature ) : ?>
						...
						?>
						<li class="my-px">
							<a data-id="setting-<?php echo esc_attr( $key ); ?>"
							   class="smooth-scroll cursor-pointer flex flex-row items-center h-12 px-4 rounded-lg text-gray-600 hover:bg-gray-100">
			...
			<input type="submit" class="hidden md:block my-3 py-3 w-full bg-primary-600 text-white rounded-md cursor-pointer shadow-sm" value="Save">
        ...
        <a id="fab-setting-scroll-to-top" class="animate__animated animate__rotateIn cursor-pointer fixed right-6 bottom-8 w-16 h-16 bg-primary-600 hover:bg-primary-700 rounded-full mouse shadow-sm transition ease-in duration-200 focus:outline-none text-center invisible md:visible" style="display:none;">
            <em class="fas fa-chevron-up text-xl text-white my-4"></em>
        </a>

		<a class="reset-option cursor-pointer md:hidden fixed right-6 bottom-24 w-16 h-16 bg-danger-600 rounded-full hover:bg-red-700 mouse shadow-sm transition ease-in duration-200 focus:outline-none text-center">
			<em class="fas fa-sync text-xl text-white my-4"></em>
		</a>

		<button type="submit" class="cursor-pointer md:hidden fixed right-6 bottom-6 w-16 h-16 bg-primary-600 rounded-full hover:bg-primary-700 active:shadow-lg mouse shadow transition ease-in duration-200 focus:outline-none">
...
<script type="text/javascript">
    ...
    /** Scroll To Top */
    jQuery('#fab-setting-scroll-to-top').click(function(){
        window.FAB_PLUGIN.scrollTo(document.documentElement, 0, 400);
    });
    window.addEventListener('scroll', function(){
        if (jQuery(window).scrollTop()<400){
            jQuery('#fab-setting-scroll-to-top').hide();
        } else {
            jQuery('#fab-setting-scroll-to-top').show();
        }
    });
</script>
